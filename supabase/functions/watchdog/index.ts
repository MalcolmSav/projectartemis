...
//   2. Safety timers past expires_at and not yet fired
//      → stamp fired_at and insert an `alarm` check-in, which the existing
//        check_ins webhook fans out to the whole circle.
//
...
async function fireExpiredTimers(
  ...
  for (const row of due as { user_id: string }[]) {
    ...
    // The check_ins INSERT webhook already fans an `alarm` out to the whole
    // circle, ignoring notification prefs and quiet hours. Reuse it rather than
    // duplicating the push logic here.
